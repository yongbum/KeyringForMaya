                    ...
                    with pm.rowColumnLayout(numberOfRows=2, rowHeight=((1, 30), (2, 30))):
                        ...
                        with pm.columnLayout(adj=True):
                            ...
                            with pm.frameLayout(label='Ctrl Color', backgroundColor=[0.4, 0.4, 0.4]):
                                pm.separator(h=10, style='out')
                                with pm.columnLayout(adj=True):
                                    pm.gridLayout(numberOfColumns=7, cellWidthHeight=(40, 40), width=300)
...
'''
import sys # 시스템 모듈을 불러줌    
import maya.cmds as cmds
sys.path.append('Q:\\yongbum\\PYTHON_PATH\\KeyringForMaya')
sys.path.append('Q:\\yongbum\\PYTHON_PATH\\KeyringForMaya\\icons')
import RenderProject 

reload(RenderProject)

from RenderProject import Riging_Tool
